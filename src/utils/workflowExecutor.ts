...
async function executeNode(
  ...
  console.log(`Executing node: ${node.name} (${node.nodeType.type})`, { inputs: inputData, parameters: node.parameters });
  
  // Check if node has necessary configuration
  if (!node.parameters || Object.keys(node.parameters).length === 0) {
    throw new Error(`Node ${node.name} is not configured properly. Please add required parameters.`);
  }
  ...
  try {
    ...
    switch (node.nodeType.type) {
      ...
      case 'api_request':
        ...
        // Mock API request
        outputData.response = { status: "success", data: { result: "API response data" } };
        break;
        
      case 'webhook':
        // Mock webhook receiver
        outputData.payload = inputData.payload || { event: "webhook_triggered", timestamp: Date.now() };
        break;
        ...
        if (Array.isArray(inputData.data)) {
          outputData.filtered = inputData.data.filter((item: any) => {
            ...
            switch (operator) {
              ...
              case 'greater_than': return Number(itemValue) > Number(value);
              case 'less_than': return Number(itemValue) < Number(value);
              default: return true;
            }
          });
        } else {
 ...
 */
export async function executeWorkflow(nodes: Node[], connections: Connection[]): Promise<{ success: boolean, results: Record<string, any>, error?: string }> {
  try {
